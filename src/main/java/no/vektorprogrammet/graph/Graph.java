...
public class Graph<V> implements IGraph<V> {
	...
	// Edges
	private final HashMap<V, HashSet<V>> adjacencyList;
	private int edges;
	 ...
	 * Loops are not allowed, addEdge(a,a) should return false
	 * 
	 * @param a,b the vertices to connect
	 * @return true if an edge was added, false otherwise
	 * @throws IllegalArgumentException if any of the two nodes is not in the graph.
	 */
		...
		if (aNeighbours.contains(b) || bNeighbours.contains(a)) {
			...
			if (!aNeighbours.contains(b) || !bNeighbours.contains(a))
				throw new IllegalStateException(
						"Data invariant is broken, " + a + " and " + b + " are only added once to adjacency set.");
	...
	public boolean removeVertex(V a) {
		...
		Iterable<V> neighboursCopy = new HashSet<V>(neighbours(a));
